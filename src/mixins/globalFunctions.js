  ...
  methods: {
    async getAllAboutSite() {
      if (localStorage.getItem('dataSite') === null) {
        const sites = await this.$axios.get(`${this.$domainOmeka}api/sites`);
        for (const site of sites.data) {
          if (site['o:slug'] === this.dataSite.slug) {
            this.dataSite.id = site['o:id'];
            this.dataSite.name = site['o:title'];
            this.dataSite.summary = this.getEmptyStringOrValue(
              site,
              'o:summary'
            )
              .replace(/\r/g, '')
        ...
        const site = JSON.parse(localStorage.getItem('dataSite'));
        ...
        if (response.data['o:navigation'] !== undefined) {
          ...
          for (const option of navigation) {
            ...
            if (type === 'page') {
              ...
              const dataItemSet = await this.$axios(url);
              type =
                dataItemSet.data['o:resource_class'] !== null
                  ? dataItemSet.data['o:resource_class']['o:id']
                  : option.type;
              title = option.data.label;
              slug = this.formatStringToUrl(option.data.label);
              this.urlVideoPage =
                type === 38 ? `/${this.dataSite.slug}/page/${slug}` : ``;
            }
            if (title !== '') {
    ...
    async getArrayMedia(data) {
      ...
      const array_coordinate = [];
      let id = 0;
      let image_data;
      // recorremos toda la media que viene de un item
      for (const datum of data['o:media']) {
        const mediaData = await this.$axios(datum['@id']);
        const media_type = this.getTypeMedia(mediaData.data);
        switch (media_type) {
          case 'image':
      ...
      if (data['o-module-mapping:marker'] !== undefined) {
        for (const coordinate of data['o-module-mapping:marker']) {
          ...
          if (coordinate['o:media'] !== null) {
            const media = await this.$axios(coordinate['o:media']['@id']);
            img = this.getMediaEmptyOrFilled(media.data);
          }
          array_coordinate.push({
            img,
            title: this.getEmptyStringOrValue(
              coordinate,
    ...
    async getAllSites(getCurrentSite = 'si') {
      ...
      let item_set_url = '';
      let media_url = '';
      const data_sites = response.data;

      if (localStorage.getItem('sites') === null) {
        for (const site of data_sites) {
          const image_urls_site = { original: '', square: '' };
          const id_site = site['o:id'];
          ...
          if (item_set.data.length > 0) {
            // is -> item set
            const is_detail_url = item_set.data[0]['o:items']['@id'];
          ...
          if (getCurrentSite === 'si') {
            ...
            getCurrentSite !== 'si' &&
            this.dataSite.slug !== site['o:slug']
          ) {
            this.sites.push({
              id: site['o:id'],
              slug: site['o:slug'],
    ...
    getAttribEmptyOrFilled(objectArray, attribName) {
      return objectArray[attribName] !== undefined
        ? objectArray[attribName][0]['@value']
        : '';
    },
    getMediaEmptyOrFilled(objectArray) {
      const media = { original: '', large: '' };
    ...
    getTypeMedia(objectArray) {
      return objectArray['o:media_type'] !== null
        ? objectArray['o:media_type'].split('/')[0]
        : objectArray['o:ingester'];
    },
