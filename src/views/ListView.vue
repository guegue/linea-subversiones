<template>
  <div
    class="blog layout-full-width mobile-tb-left button-stroke header-transparent header-fw minimalist-header sticky-header sticky-dark ab-hide subheader-both-center menuo-right menuo-no-borders footer-copy-center"
  >
    <div id="Wrapper">
      ...
      <header
        ...
        :show-title-description="title.length > 0"
        :title="title"
      ></header>
      <container
        :contents="contents"
    ...
    async getContentFromPage(urlPage) {
      ...
      if (answer.data['o:block'] != null) {
        for (const detail of answer.data['o:block']) {
          const typeLayout = detail['o:layout'].toLowerCase();
          let size_item_set = 0;
          let url_item = '';
          ...
          if (typeLayout === 'itemwithmetadata') {
            ...
            for (const data of detail['o:attachment']) {
              const url_media = data['o:media']['@id'];
              ...
              this.littleArray.push({
                ...
                url_img: this.getMediaEmptyOrFilled(
                  responseMedia.data,
                  'o:original_url'
                ).original,
                url_img_large: responseMedia.data['o:thumbnail_urls'].large,
                date: this.getAttribEmptyOrFilled(
                  responseItem.data,
                  'dcterms:date'
                ),
                author: this.getAttribEmptyOrFilled(
                  responseItem.data,
                  'bibo:citedBy'
                ),
              });
            ...
            for (const data of detail['o:attachment']) {
              ...
              const media = await this.getFirstImageFound(responseItem.data);
              this.littleArray.push({
                id: responseItem.data['o:id'],
                title: this.getAttribEmptyOrFilled(
                  responseItem.data,
        ...
        const itemSetUrl = answer.data['o:items']['@id'];
        const dataItemSet = await this.$axios(itemSetUrl);

        // guardo la descripcion del conjunto de items
        this.summary = this.getAttribEmptyOrFilled(
          answer.data,
          'dcterms:description'
        );
        const size_item_set = dataItemSet.data.length;
        ...
        for (const data of dataItemSet.data) {
          ...
          if (data['o:media'] !== null) {
            const image = await this.getFirstImageFound(data);
            this.littleArray.push({
              id: data['o:id'],
        ...
        if (answer.data['dcterms:isPartOf'] !== undefined) {
          for (const related of answer.data['dcterms:isPartOf']) {
            const type_resource = related.value_resource_name;
            let url = '';
            const id_resource = related.value_resource_id;
